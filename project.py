...
app.config['GOOGLE_LOGIN_CLIENT_ID'] = \
    '1082999837227-rc60e82b4rfpucevhs8dadoahvdj08v8.apps.googleusercontent.com'
app.config['GOOGLE_LOGIN_CLIENT_SECRET'] = 'W58ZcXA3ynkdsg8t9DxhwT44'
...
def latestItems():
    ...
    items = \
        session.query(Item).order_by(desc(Item.creation_date)).limit(10).all()
...
def listItems(category_id):
    ...
    return render_template('list_items.html', categories=categories,
                           category=category,
                           number_of_items=len(category.items),
...
def itemPicture(item_id):
    ...
    return (item.picture_data, 200, {'Content-Type': content_type,
...
def createItem():
    ...
    existingItem = \
        session.query(Item).filter_by(category_id=category.id,
            name=name).first()
    if existingItem:
        flash('An item with the same name already exists in this category. Please choose a different name'
              , 'danger')
        return render_template('create_item.html',
                               categories=categories)
...
def deleteItem(item_id):
    ...
    if not useNonce(nonce):
        ...
        return render_template('delete_item.html',
                               categories=categories, item=item,
                               nonce=createNonce())  # ToDo error message
...
if __name__ == '__main__':
    app.secret_key = \
        'jIdee28q8BzcqR3oPmKICRkCYUTs5fn3cHOpoBbLMMbuK0B/Nhwd//hrYkwUD6CTAhlwo7jiuNnFfiMrf36m/g=='
    app.debug = True
